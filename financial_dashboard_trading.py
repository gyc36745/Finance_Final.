...
choice = st.selectbox('選擇金融商品', choices, index=0,key='select_asset')
##### 读取Pickle文件
...
with st.expander("策略參數最佳化", expanded=True):
	#%%最佳化
...
# 顯示最佳結果（放在 expander 外面）
if df_result is not None and best_params is not None:
    if choice_strategy == choices_strategies[0]:
        st.success(f"最佳參數：短MA={best_params[0]}, 長MA={best_params[1]}, 停損={best_params[2]}，總獲利={best_profit:.2f}")
        st.subheader("所有參數組合績效")
    ...
    st.dataframe(df_result.sort_values(by=df_result.columns[-1], ascending=False).reset_index(drop=True))

    # Altair 圖
    import altair as alt
    chart = alt.Chart(df_result).mark_circle(size=60).encode(
        x=df_result.columns[0] + ":Q",
        y=df_result.columns[1] + ":Q",
        color=df_result.columns[-1] + ":Q",
        tooltip=list(df_result.columns)
    ).interactive()
